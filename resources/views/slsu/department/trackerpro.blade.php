...
<div class="container">
    <h4>Track {{$reg->StudentNo}} ({{utf8_decode($reg->FirstName.' '.$reg->LastName)}})</h4>
    <div class="row">
        <div class="col-md-12">
            <div class="main-timeline4">
                <div class="timeline">
                    <a href="#" class="timeline-content">
                        ...
                        <div class="inner-content">
                            <h3 class="title">SUBMIT CONSENT</h3>
                            <p class="description">
                                Submitted: {{date('F j, Y', strtotime($reg->DateEncoded))}}<br>
                ...
                <div class="timeline">
                    <a href="#" class="timeline-content">
                        ...
                        <div class="inner-content">
                            <h3 class="title">DEPARTMENT / FACULTY</h3>
                            <p class="description">
                                @if(!empty($reg->DateEnrolled))
                                  ENCODED: {{date('F j, Y', strtotime($reg->DateEnrolled))}}<br>
                                  BY: {{$reg->EnrollingOfficer}}<br>
                                  Status: <span class = 'text-success'>DONE</span>
                                @else
                                  @if ($reg->finalize == 2)
                                    Waiting for year level adviser to process enrollment.<br>
                                    Status: <span class = 'text-danger'>PENDING</span>
                                  @else
                                    Unknown status. Please inform nestnie
                                  @endif
                                @endif
                            </p>
                        </div>
                ...
                <div class="timeline">
                    <a href="#" class="timeline-content">
                        ...
                        <div class="inner-content">
                            <h3 class="title">FHE</h3>
                            <p class="description">
                                @if ($reg->SchoolLevel == "Under Graduate")
                                  @if(!empty($reg->TESDate))
                                      APPROVED: {{date('F j, Y', strtotime($reg->TESDate))}}<br>
                                      BY: {{$reg->TESBy}}<br>
                                      Type: {{($reg->TES == 1?'FHE':'NON-FHE')}}<br>
                                      Status: <span class = 'text-success'>DONE</span>
                                  @else
                                      @if ($reg->finalize == 0 and $reg->TES == 0)
                                        Waiting for FHE incharge to process the request.<br>
                                        Status: <span class = 'text-danger'>PENDING</span>
                                      @else
                                        Waiting for the other steps to be completed.<br>
                                        Status: <span class = 'text-danger'>WAITING</span>
                                      @endif
                                  @endif
                                @else
                                  NOT APPLICABLE<br>
                ...
                <div class="timeline">
                    <a href="#" class="timeline-content">
                        ...
                        <div class="inner-content">
                            <h3 class="title">CASH OFFICE</h3>
                            <p class="description">
                                @if(!empty($reg->CashierDate))
                                    ASSESSED: {{date('F j, Y', strtotime($reg->CashierDate))}}<br>
                                    BY: {{$reg->Cashier}}<br>
                                    Fee: {{number_format($reg->Balance,2)}}<br>
                                    Status: <span class = 'text-success'>DONE</span>
                                @else
                                    @if ($reg->finalize == 0)
                                      @if ($reg->SchoolLevel == "Masteral" or $reg->SchoolLevel == "Doctoral")
                                        Waiting for Cashier incharge to assess your tuition and miscellaneous fees.<br>
                                        Status: <span class = 'text-danger'>PENDING</span>
                                      @elseif($reg->TES != 0)
                                        Waiting for Cashier incharge to assess your tuition and miscellaneous fees.<br>
                                        Status: <span class = 'text-danger'>PENDING</span>
                ...
                <div class="timeline">
                    <a href="#" class="timeline-content">
                        ...
                        <div class="inner-content">
                            <h3 class="title">REGISTRAR's OFFICE</h3>
                            <p class="description">
                                @if(!empty($reg->DateValidated))
                                    VALIDATED: {{date('F j, Y', strtotime($reg->DateValidated))}}<br>
                                    BY: {{$reg->ValidatedBy}}<br>
                                    Status: <span class = 'text-success'>DONE</span>
                                @else
                                    @if ($reg->finalize == 5)
                                      Waiting for registrar's office incharge to validate your enrolment.<br>
                                      Status: <span class = 'text-danger'>PENDING</span>
                                    @else
                                      Waiting for the other steps to be completed.<br>
                                      Status: <span class = 'text-danger'>WAITING</span>
