  ...
  <table class="table table-sm table-hover">
    ...
    <tbody>
      ...
      @foreach($daysWeek as $day)
        <tr>
              <td colspan = "{{$size}}" class = "fw-bold text-primary">{{strtoupper($day)}}</td>
        </tr>
        @foreach($allDays as $oneDay)
          @if (strtolower($oneDay['day']) == strtolower($day))
              <?php

              $bg = '';
              if (in_array($oneDay['ID'], $withpareha))
                  $bg = 'text-warning ';
              ?>
              <tr class = "{{$bg}}">
                @foreach($lists as $list)
                  @if($list->id == $oneDay['ID'])
                ...
                @foreach($lists as $list)
                  ...
                  @endif
                @endforeach
                <td class = "text-nowrap">{{$oneDay['Room']}}</td>
                <td class = "text-nowrap">{{$oneDay['Time']}}</td>
              </tr>
          @endif
        @endforeach
