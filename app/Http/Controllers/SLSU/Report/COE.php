...
class COE extends TCPDF
{
    ...
    function Content()
      ...
      $this::Ln();
      $html = '<div style="text-align:justify;line-height: 35px;text-indent: 30px;">THIS IS TO CERTIFY that <b>'.($student->Sex=="M"?"MR.":"MS.").' '.strtoupper(utf8_decode($student->FirstName.(empty($student->MiddleName)?" ":" ".$student->MiddleName[0].". ").$student->LastName)).'</b> is a bonafide '.$this->getYear($reg->StudentYear). " year student in the <strong>".strtoupper($reg->course->course_title)." (".strtoupper($reg->course->accro) .")</strong>"
                  .(empty($reg->major->course_major)?"":" major in <strong>".strtoupper($reg->major->course_major)."</strong>")." in this University, ".GENERAL::Semesters()[$this->getSem()]['Long'].", Academic Year <strong>".GENERAL::setSchoolYearLabel($this->getSy(),$this->getSem()).".</strong></div>";
      $this::writeHTML($html, true, 0, true, true);


      $html = '<div style="text-align:justify;line-height: 35px;text-indent: 30px;">This certification is issued to <strong>'.($student->Sex=="M"?"Mr":"Ms").'. '.ucwords(strtolower(utf8_decode($student->FirstName.(empty($student->MiddleName)?" ":" ".$student->MiddleName[0].". ").$student->LastName))).'</strong> for '. $this->getReason(). ' purpose only.</div>';
      $this::writeHTML($html, true, 0, true, true);
