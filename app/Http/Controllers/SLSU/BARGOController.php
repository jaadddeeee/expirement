...
class BARGOController extends Controller
{
    ...
    public function add(Request $request){
        ...
        $ex = CLBARGO::where("StudentNo", $dataTMP[0])
        ...
        try{
            ...
            foreach ( $row_range as $row ) {
                ...
                if (empty($studentno) or empty($sy) or empty($sem) or empty($reason)){
                    ...
                    $ex = CLBARGO::where("StudentNo", $studentno)
                      ->where("Description", $reason)
                      ->where("SchoolYear", $sy)
                      ->where("Semester", $sem)
                      ->first();
    ...
    public function studentsearch(Request $request){
      ...
      $depts = CLBARGO::select('clearance_bargo.*')
        ->leftjoin('students as s', "clearance_bargo.StudentNo", "=", 's.StudentNo')
        ->where(function($q) use ($id){
            $q->Orwhere("clearance_bargo.StudentNo","LIKE","%{$id}%")
