...
class OSASController extends Controller
{
    ...
    public function add(Request $request){
        ...
        $ex = CLOSAS::where("StudentNo", $dataTMP[0])
        ...
        if ($ins){
          ...
          $data = [
            "Description" => "One entry has been added to your OSAS clearance",
            "StudentNo" => $dataTMP[0],
            "AddedBy" => Auth::user()->Emp_No,
    ...
    public function remove(Request $request){
      ...
      if ($del){
        ...
        $data = [
          "Description" => "One entry has been removed from SAS clearance entry",
        ...
        try{
            ...
            foreach ( $row_range as $row ) {
                ...
                if (empty($studentno) or empty($sy) or empty($sem) or empty($reason)){
                    ...
                    $ex = CLOSAS::where("StudentNo", $studentno)
                      ->where("Description", $reason)
                      ->where("SchoolYear", $sy)
                      ->where("Semester", $sem)
                      ->first();
                    ...
                    if (!empty($ex)){
                        ...
                        if ($ins){
                            ...
                            $data = [
                              "Description" => "One entry has been added to your SAS clearance",
                              "StudentNo" => $studentno,
                              "AddedBy" => Auth::user()->Emp_No,
                              "created_at" => date('Y-m-d h:i:s')
                            ];
    ...
    public function studentsearch(Request $request){
      ...
      $depts = CLOSAS::select('clearance_osas.*')
