...
class LibraryController extends Controller
{
    ...
    public function add(Request $request){
        ...
        $ex = CLLibrary::where("StudentNo", $dataTMP[0])
        ...
        if ($ins){
          ...
          $data = [
            "Description" => "One entry has been added to your library clearance",
            "StudentNo" => $dataTMP[0],
            "AddedBy" => Auth::user()->Emp_No,
        ...
        try{
            ...
            foreach ( $row_range as $row ) {
                ...
                if (empty($studentno) or empty($sy) or empty($sem) or empty($reason)){
                    ...
                    $ex = CLLibrary::where("StudentNo", $studentno)
                      ->where("Description", $reason)
                      ->where("SchoolYear", $sy)
                      ->where("Semester", $sem)
                      ->first();
    ...
    public function studentsearch(Request $request){
      ...
      // $depts = DB::connection(strtolower(session('campus')))
      //     ->table('clearance_library as cl', 's.FirstName', 's.LastName')
      //     ->select('cl.*')
      //     ->leftjoin('students as s', "cl.StudentNo", "=", 's.StudentNo')
      //     ->whereOr("cl.StudentNo","LIKE","%{$id}%")
      //     ->whereOr("s.LastName","LIKE","%{$id}%")
      //     ->whereOr("s.FirstName","LIKE","%{$id}%")
      //     ->get();

      // dd($depts);

      $depts = CLLibrary::select('clearance_library.*')
        ->leftjoin('students as s', "clearance_library.StudentNo", "=", 's.StudentNo')
        ->where(function($q) use ($id){
            $q->Orwhere("clearance_library.StudentNo","LIKE","%{$id}%")
            ->Orwhere("s.LastName","LIKE","%{$id}%")
            ->Orwhere("s.FirstName","LIKE","%{$id}%");
        })
        ->get();

        return view('slsu.clearance.customlibrary', compact('depts'));
    }
