...
class MISController extends Controller
{
    ...
    public function add(Request $request){
        ...
        $ex = CLMIS::where("StudentNo", $dataTMP[0])
        ...
        if ($ins){
          ...
          $data = [
            "Description" => "One entry has been added to your MIS clearance",
            "StudentNo" => $dataTMP[0],
            "AddedBy" => Auth::user()->Emp_No,
        ...
        try{
            ...
            foreach ( $row_range as $row ) {
                ...
                if (empty($studentno) or empty($sy) or empty($sem) or empty($reason)){
                    ...
                    $ex = CLMIS::where("StudentNo", $studentno)
                      ->where("Description", $reason)
                      ->where("SchoolYear", $sy)
                      ->where("Semester", $sem)
                      ->first();
    ...
    public function studentsearch(Request $request){
      ...
      $depts = CLMIS::select('clearance_mis.*')
      ->leftjoin('students as s', "clearance_mis.StudentNo", "=", 's.StudentNo')
      ->where(function($q) use ($id){
          $q->Orwhere("clearance_mis.StudentNo","LIKE","%{$id}%")
          ->Orwhere("s.LastName","LIKE","%{$id}%")
          ->Orwhere("s.FirstName","LIKE","%{$id}%");
      })
      ->get();

      return view('slsu.clearance.custommis', compact('depts'));
    }
