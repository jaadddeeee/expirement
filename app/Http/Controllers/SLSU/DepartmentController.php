...
class DepartmentController extends Controller
{
    ...
    public function enrolment(Request $request){
      ...
      try{
        ...
        if (!empty($request->id)){
          $step1s->where("registration.StudentNo", "LIKE", "%".$request->id."%");
        }
        $statuss = Status::all();

        $step1s = $step1s->orderby("DateEncoded")
            ->orderby("TimeEncoded")
            ->paginate(50);
        if ($request->ajax()){
          return view('slsu.department.step2res', compact('step1s','statuss'));
        }else{
    ...
    public function proenrolment(Request $request){
        ...
        try{
          ...
          if (!GENERAL::isCleared($studentno)){
          ...
          return view('slsu.enrol.index', compact('subjects','one','grades','credits','reg','statuss'),[
            'pageTitle' => $pageTitle,
            'headerAction' => $headerAction,
    ...
    public function enrolmenttracker(Request $request){
      try{
        ...
        if ($request->ajax()){
          ...
          $reg = Registration::select('registration.*','s.LastName','s.FirstName')
              ->leftjoin('students as s', 'registration.StudentNo', '=','s.StudentNo')
              ->where("registration.StudentNo", $request->StudentNo)
              ->where('registration.SchoolYear', $request->SchoolYear)
              ->where('registration.Semester', $request->Semester)
              ->first();

          if (empty($reg)){
            throw new Exception("Enrolment not found.");
          }
          return view('slsu.department.trackerpro', compact('reg'));
        }else{
          $pageTitle = "ENROLMENT TRACKER";
          $headerAction = '<a href="javascript:history.back()" class="btn btn-sm btn-primary" role="button">Back</a>';
          return view('slsu.department.tracker', [
            'pageTitle' => $pageTitle,
            'headerAction' => $headerAction,
          ]);
        }
    ...
    public function list(Request $request){
        try{
          ...
          $all = DB::connection(strtolower($campus))
            ->table('department')
            ->where('Active', 0)
